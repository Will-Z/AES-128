...
static void calc_AM(int num);   //计算 A逆
static void calc_rAM(int num);
...
int A[8][8];                //仿射矩阵
int AM[8][8];               //A的逆
int rAM[256][8][8];          //a0 = i时, 计算出的A^-1 存放于rAM[i][][]中
int AM_dic[8][8][8];
...
*///--------------------------------------------------------------- 计算AM_dic
  /*  for (int num = 0; num < 8; num++) {
        ...
        for (int i = 0; i < 8; i++)
            for (int j = 0; j < 8; j++)
                AM_dic[num][i][j] = AM[i][j];

        printf("---------------------------------  %d \n\n", num);
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++)
                printf("%d ", AM_dic[num][i][j]);
            printf("\n");
        }
...
/*    // 从文件读入AM_dic[][]
    FILE *fin;
    fin = fopen("/Users/Will/Programming/Clion/prac/AM.txt", "r");
    for (int num = 7; num >= 0; num--)
        for (int i = 0; i < 8; i++)
            for (int j = 0; j < 8; j++)
                fscanf(fin, "%d", &AM_dic[num][i][j]);

  */
    int ans_key[256][16];
    int ans_sbox[256][256];

    for (int num = 1; num < 256; num++) {
        calc_rAM(num);
        for (int i = 0; i < 8; i++)
            for (int j = 0; j < 8; j++)
                rAM[num][i][j] = temp[i][j];                 //a0=num 对应的A^-1存在rAM中.
        ...
        for (int i = 1; i < 16; i++)
            ans_key[num][i] = kk[i] ^ ans_key[num][0];


        for (int i = 0; i < 256; i++)
            ans_sbox[num][i ^ ans_key[num][0]] = affine(SS[i]);
